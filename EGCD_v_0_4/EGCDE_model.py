...
class ETR_loss_HC(nn.Module):
    def __init__(self):
        super().__init__()

    def forward(self, A, C,X,Y):
        n, k = list(C.size())
        IsumC = torch.ones(1, n)
        IsumCDC = torch.ones(k, 1)
        ...
        # constraint = 0.001*(torch.norm( (k/n)*(C.t().mm(C)) - torch.eye(k)))
        # total=total + constraint
        X_recon=0.0001*torch.norm(X-Y)
        return total+X_recon
def use_EGCDE(features_in,adj_in,c_num,feat_map='dot',neigh=20):
    #基础设置，学习率、迭代次数
    learning_rate=0.1
    epochs=1000
    #weight_decay=5e-4
    ...
    model_en=EGCD_Net(X_dim,c_num).to(device)
    model_de=Decoder_Net(c_num,X_dim).to(device)
    #loss改为自定义loss
    criterion = ETR_loss_HC()
    #Adam优化器
    optimizer=optim.Adam(itertools.chain(model_en.parameters(),model_de.parameters()),lr=learning_rate,amsgrad=True)
    lr_optim=torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.8, patience=3, verbose=False,
                                               threshold=0.000001, threshold_mode='rel', cooldown=0, min_lr=1e-6, eps=1e-08)
    #数据处理
    ...
    for epoch in range(epochs):
        gc.collect()
        h,result=model_en(tensor_adj,tensor_x)
        y=model_de(tensor_adj,h)
        loss = criterion(tensor_loss_adj, result,tensor_x,y)
...
if __name__ == '__main__':
    ...
    print(y.shape)
    [_,k]=y.shape
    label,loss=use_EGCDE(x, a, k,feat_map='dot',neigh=25)
    print(loss)
    print(label)
