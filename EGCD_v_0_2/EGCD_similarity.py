...
import numpy as np
import itertools
from EGCD_parts import GCN_layers
from EGCD_parts import Loss_func
from EGCD_parts import CDN_layers
...
def use_EGCD(features_in,adj_in,labels_in,feat_map='dot',mask=False,printloss=False):

	A = adj_in.astype("float32")
	A_0=constru_AW(A,features_in,feat_map,mask)
	...
	#选择adam优化器
	gd = torch.optim.Adam(itertools.chain(gcn.parameters(),cdn.parameters()),lr=0.001,amsgrad=True)#,weight_decay=0.1,amsgrad=True	loss_ram=[]#训练loss存储
	#训练
	loss_ram=[]
...
def use_EGCD_H(features_in,adj_in,labels_in,feat_map='dot',mask=False,printloss=False):
	A = adj_in.astype("float32")
	A_0=constru_AW(A,features_in,feat_map,mask)
	#A需要正规化
	A1=GCN_layers.normalize(torch.FloatTensor(A_0.astype("float32")),True)#邻接矩阵转tensor
	...
	cdn= CDN_layers.CDN(k,k)#全连接部分
	#选择adam优化器
	gd = torch.optim.Adam(itertools.chain(gcn.parameters(),cdn.parameters()),lr=0.01)#,weight_decay=0.1,amsgrad=True	loss_ram=[]#训练loss存储
	#训练
	loss_ram=[]
	h_papra=1
	for i in range(500):
		...
		y_pred ,y_original =cdn(H)
		k, y_out = y_pred.max(1)
		criterion = Loss_func.ETR_loss_H()
		loss,h_part = criterion(A1, y_pred,H,h_papra,i)
		if i<=10:
			h_papra=-0.001*loss.detach()/h_part.detach()
		#print(h_papra)
		#loss_final=loss+h_papra*h_part
		if i%100==0:
			for p in gd.param_groups:
				p['lr'] *=0.5
		loss.requires_grad_(True)
